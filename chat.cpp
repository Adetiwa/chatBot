...
void CBot::save_log()
{
	...
	if(logfile.fail())
	{
		...
		logfile << "\n\nConversation log  with " << username() <<" - " << ctime(&ltime) << std::endl;
	}
}
...
{
	std::string file = std::to_string(m_Emotions)+"_script.txt";
	std::fstream fin(file, std::ios::in);
	if(fin.fail())
	{
		throw std::string("Unable to load the database for this emotion --- "+getEmo()+" . Filename is "+file);
	}
...
void CBot::get_input()
{
	...
	for (int i = 0; i < 4; i++) {
		std::string thisInsult = insults[i];
		if (m_sInput.find(thisInsult) != std::string::npos) {
			decreaseEmo();
		}
	}
}
...
void CBot::preprocess_response()
{
	...
	if(m_sResponse.find("*") != std::string::npos)
	{
	 ...
	 if (m_sResponse.find("(((") != std::string::npos) {
		int first = m_sResponse.find("(((");
		int last = m_sResponse.find(")))");
		std::string strNew = m_sResponse.substr (first,last-first);
		//std::cout<<"Sub shit " << strNew << std::endl;

		if (strNew == "(((getEmo") {
				action = getEmo();
				replace(m_sResponse, "(((", " ");
				replace(m_sResponse, ")))", " ");
				replace(m_sResponse, "getEmo", action);
		} else if (strNew == "(((getBot") {
			action = botName();
			replace(m_sResponse, "(((", " ");
			replace(m_sResponse, "GETBOT", action);
		} else if(strNew == "(((username") {
			trimRight(m_sInput, ".");
			UpperCase(m_sInput);
			tokenize(m_sInput, ListOfWord);
			bestKeyWord = findBestKey(ListOfWord);
			setUserName(bestKeyWord);
			userhasname = 1;
			action = username();
			replace(m_sResponse, "(((", " ");
			replace(m_sResponse, ")))", " ");
			replace(m_sResponse, "username", "HELLO "+action);
			save_log();
		} else {
			action = "null";
			replace(m_sResponse, "(((", " ");
	 ...
	 if (m_sResponse.find("++") != std::string::npos) {
		int first = m_sResponse.find("++");
		int end = m_sResponse.find("))");
		std::string strNew = m_sResponse.substr(first,end-first);

							if (strNew == "++questionN") {
								replace(m_sResponse, "++questionN))", " ");
								handle_event("BOT ASK QUESTION NAME**");
								//preprocess_response();
							}
...
void CBot::find_match()
{
	...
	else
	{
	 ...
	 for(auto const& x : KnowledgeBase) {
		std::string const& key = x.first;
		std::vector<CResponse> const& value = x.second;
		//std::cout <<  key <<  std::endl;
		std::vector<CResponse>::const_iterator iter = value.begin();
		for( ; iter != value.end(); ++iter )
		{
		  vstring ListOfContext = iter->getRespList();
		  for (auto i : ListOfContext) {
		  //  std::cout <<  i << std::endl;
		  }
		}
	}
...
int main()
{
	...
	try
	{
		...
		while(!bot.quit())
		{
			//bot.loadDatabase();
			bot.get_input();
		//s	std::cout << "Current event is " << bot.getEvent() << std::endl;
		if (bot.getUsernamestate()) {
			bot.save_log(bot.username());
		}
			bot.respond();
		}
